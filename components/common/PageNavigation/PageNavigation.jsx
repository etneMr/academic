...
export function PageNavigation({ selected, length, skip, total, limit, onPageClick = () => { } }) {
    const rows = [];
    rows.push(
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="33" viewBox="0 0 32 33" fill="none" onClick={() => onPageClick(selected - 1)} key={-1}>
    ...
    for (let i = pos; i < pos + 3; i++) {
        ...
        if (i === selected) {
            rows.push(
                <div className="page-navigate-item" key={i + 1} style={{ background: "#4D44B5", borderColor: "#4D44B5" }}>
            ...
            rows.push(
                <div className="page-navigate-item" key={i + 1} onClick={() => onPageClick(i)}>
    ...
    if (selected !== length - 1)
        rows.push(<svg xmlns="http://www.w3.org/2000/svg" width="32" height="33" viewBox="0 0 32 33" fill="none" onClick={() => onPageClick(selected + 1)} key={-2}>
    ...
    return (<>
        <div id="page-navigation-footer">
            <div className="display-total-students">
                Showing <span className="number">{skip + 1}-{skip + limit}</span> from <span className="number">{total}</span> data
            </div>
